...
def check_and_prepare_args(args):
    ...
    args.num_updates = int(args.num_env_steps) // args.num_steps // args.num_processes
...
def clear_port(port,ask):
    ...
    '''get pid'''
    import subprocess
    command = 'ps axf | grep \'port {}\' | grep -v grep | awk \'{{print $1}}\''.format(port)
    pid = subprocess.check_output([command],shell=True,stderr=subprocess.STDOUT).decode("utf-8").split('\n')[0]
