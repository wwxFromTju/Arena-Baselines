...
class MultiAgentCluster(object):
    ...
    def store(self):
        ...
        if self.checkpoints_reward_record is not None:
            ...
            new_record = np.array([[self.learning_agents[0].current_checkpoint_by_frame,0.0]])
            ...
            try:
                ...
                np.save(
                    os.path.join(self.log_dir, "checkpoints_reward_record.npy"),
                    self.checkpoints_reward_record,
                )
                print('# INFO: Store checkpoints_reward_record ok: {}'.format(self.checkpoints_reward_record.shape))
    ...
    def reload_playing_agents(self):
        ...
        if self.reload_playing_agents_principle in ['prioritized']:
            ...
            probability_to_checkpoints = scipy.special.softmax(
                -self.checkpoints_reward_record[:,self.checkpoints_reward_record_REWARD],
                axis = 0,
            )
            checkpoint_picked_by_frame = np.random.choice(
                self.checkpoints_reward_record[:,self.checkpoints_reward_record_FRAME],
                p = probability_to_checkpoints,
            )
            checkpoint_picked_by_frame = int(checkpoint_picked_by_frame)
            checkpoint_picked_by_index = np.where(self.checkpoints_reward_record==checkpoint_picked_by_frame)[0][0]
            print('# INFO: Pick checkpoint {} from {} records, the probability this pick is {} (uniform is {}), the reward recorded is {}'.format(
                ...
                self.checkpoints_reward_record[checkpoint_picked_by_index,self.checkpoints_reward_record_REWARD]
            ))
