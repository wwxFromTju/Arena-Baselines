...
class Agent(object):
    ...
    def schedule_trainer(self):
        ...
        if self.use_linear_lr_decay:
            ...
            if self.trainer_id == "acktr":
                ...
                update_linear_schedule(self.trainer.optimizer, self.update_i, self.num_updates, self.trainer.optimizer.lr)
            else:
                update_linear_schedule(self.trainer.optimizer, self.update_i, self.num_updates, self.lr)
    ...
    def update(self):
        ...
        '''prepare for update'''
        with torch.no_grad():
            obs, recurrent_hidden_states, masks = self.rollouts.get_policy_inputs(-1)
        ...
        '''update'''
        value_loss, action_loss, dist_entropy = self.trainer.update(self.rollouts)
    ...
    def to_print_str(self,mode):
        ...
        if mode in ['learning']:
            ...
            FPS = ((self.get_num_trained_frames()+self.num_processes * self.num_steps)-self.num_trained_frames_start) / (time.time() - self.time_start)
    ...
    def restore(self,principle='recent'):
        ...
        try:
            ...
            print('# WARNING: [Agent {}][Restore failed: {}]'.format(self.id,e))
    ...
    def get_possible_checkpoints(self):
        ...
        for file in glob.glob(os.path.join(self.log_dir, 'agent_*.pt')):
            possible_checkpoints += [int(file.split(self.log_dir)[1].split('.pt')[0].split('agent_')[1])]
